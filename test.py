    ...
    messages = [
        ...
        },
        {
            ...
            "content": f"Organize the following CV text into a structured JSON format containing:\n"
                       ...
                       f"    * 'end_date' (graduation year or the year the degree was obtained. If a specific month and year are found, use 'MM.YYYY'; otherwise, just 'YYYY'),\n"
                       f"    * 'degree' (e.g., Master, Bachelor. Extract the most appropriate degree name or equivalent if available),\n"
                       f"    * 'field' (e.g., Computer Science. Use contextual information from the CV to determine the field),\n"
                       f"    * 'institution' (e.g., University of Lyon. Extract the full name of the institution as found in the CV),\n"
                       f"    * 'format' (e.g., alternance, or leave empty if not specified))\n"
                       f"    Ensure all information is accurately extracted based on the text provided in the CV and infer missing data where possible."
