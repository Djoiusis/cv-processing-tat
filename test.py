    ...
    messages = [
        ...
        },
        {
            ...
            "content": f"Organize the following CV text into a structured JSON format containing:\n"
                       ...
                       f"    * 'end_date' (graduation year or the year the degree was obtained. If a specific date is missing, leave the field blank),\n"
                       f"    * 'degree' (e.g., Master, Bachelor. Extract the degree directly from the CV text, without adding inferred text like 'Non spécifié'),\n"
                       f"    * 'field' (e.g., Computer Science. Extract the field of study directly from the CV),\n"
                       f"    * 'institution' (e.g., University of Lyon. Use the exact name as it appears in the CV),\n"
                       f"    * 'format' (e.g., alternance, or leave empty if not mentioned))\n"
